...
def lidar_data(point_cloud, lidar_point_cloud_buffer):
    mat_data = np.copy(no.frombuffer(point_cloud.raw_data, dtype=np.dtype('f4')))
    reshape_of_image = np.reshape(mat_data, (int(mat_data.shape[0] / 4), 4))
...
def generate_lidar_blueprint(blueprint_library):
    mylidar = blueprint_library.find('sensor.lidar.ray_cast')
    mylidar.set_attribute('channels', str(32))
    mylidar.set_attribute('points_per_second', str(50000))
    mylidar.set_attribute('rotation_frequency', str(50))
    mylidar.set_attribute('range', str(20))
    return mylidar
